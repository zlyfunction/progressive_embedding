...
void check_total_angle(const Xd &uv, const Xi &F, const Xi &cut, const Vd &Sn, const std::string &name)
{
    ...
    Vd angles_total(uv.rows());
    angles_total.setConstant(0);
    for (int i = 0; i < angles_total.rows(); i++)
    {
        for (auto v : VV[i])
            angles_total(i) += angles(v);
    }
...
int main(int argc, char *argv[])
{
    ...
    if (cmdl[{"-h", "-help"}])
    {
        ...
        std::cout << "-uv: input uv for opt only" << std::endl;
        std::cout << "-c: opt steps" << std::endl;
        exit(0);
    }

    int threshold;
    std::string model, outfile, file_for_opt;
    bool use_harm, space_filling_curve;
    int total_steps;
    cmdl("-in") >> model;
    cmdl("-uv") >> file_for_opt;
    cmdl("-c") >> total_steps;
    ...
    if (file_for_opt == "no")
    {
        Eigen::MatrixXd uv, uv_test, uv_out;
        Eigen::MatrixXi F_uv;
        Eigen::VectorXi S, JJ;
        Vd SS;
        ...
        igl::serialize(V, "V", name + "_foropt", true);
        igl::serialize(uv_new, "uv", name + "_foropt");
        igl::serialize(cut, "cut", name + "_foropt");
        igl::serialize(F, "F", name + "_foropt");
    ...
    else
    {
        ...
        igl::deserialize(V, "V", file_for_opt);
        igl::deserialize(uv_new, "uv", file_for_opt);
        igl::deserialize(cut, "cut", file_for_opt);
        igl::deserialize(F, "F", file_for_opt);
        std::cout << V.rows() << " " << uv_new.rows() << " " << cut.rows() << " " << F.rows() << std::endl;
    }
    ...
    std::ofstream write_csv(name + "_log.csv");

    // remember to set NOHESSIAN
    auto do_opt_gd = [&F, &Aeq, &AeqT, &G, &dblarea, &compute_energy, &compute_energy_max, &write_csv](Eigen::MatrixXd &cur_uv, int start_it, int end_it) {
        bool flag = false;
        double energy = compute_energy(cur_uv);
        double energy_old = -1;
        clock_t t, iter_start_time, it_end_time;
        spXd hessian(cur_uv.rows() * 2, cur_uv.rows() * 2);
        hessian.setIdentity(); // do gd
        ...
        for (int ii = start_it; ii < end_it; ii++)
        {
            ...
            t = clock();
            get_grad_and_hessian(G, dblarea, cur_uv, grad, hessian_tmp);
            std::cout << "get grad time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
            ...
            std::cout << "solve time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
            ...
            double E_max = compute_energy_max(cur_uv);
            std::cout << std::setprecision(20) << "E_avg= " << energy << "\tE_max = " << E_max << std::endl;
            std::cout << "grad.norm() = " << gradL.norm() << std::endl;
            it_end_time = clock();
            std::cout << "one iter total_time = " << (double)(it_end_time - iter_start_time) / CLOCKS_PER_SEC << std::endl
                      << std::endl;

            write_csv << std::setprecision(20) << ii << "," << energy << "," << E_max << "," << (double)(it_end_time - iter_start_time) / CLOCKS_PER_SEC << std::endl;
    ...
    auto do_opt_newton = [&F, &Aeq, &AeqT, &G, &dblarea, &compute_energy, &compute_energy_max](Eigen::MatrixXd &cur_uv, int N) {
        double energy = compute_energy(cur_uv);
        double energy_old = -1;
        clock_t t;
        int gd_step = 200;
        double step_size_last_it = 1;
        std::vector<bool> do_gd(N + gd_step, false);
        Eigen::SparseLU<Eigen::SparseMatrix<double>> solver;
        for (int ii = 0; ii < N; ii++)
        {
            ...
            std::cout << "get grad and hessian time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
            t = clock();
            if (step_size_last_it == 0)
            {
                for (int kk = 0; kk < gd_step; kk++)
                    do_gd[ii + kk] = true;
            }
            if (do_gd[ii])
            {
                std::cout << "do gradient descent" << std::endl;
                hessian.setIdentity();
            }

            spXd kkt(hessian.rows() + Aeq.rows(), hessian.cols() + Aeq.rows());
            buildkkt(hessian, Aeq, AeqT, kkt);
            std::cout << "build kkt time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
            ...
            solver.factorize(kkt);
            std::cout << "solver.info = " << solver.info() << std::endl;
            std::cout << "factorize time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
            ...
            t = clock();
            energy = bi_linesearch(F, cur_uv, new_dir, compute_energy, grad, energy);
            std::cout << "line_search time = " << (double)(clock() - t) / CLOCKS_PER_SEC << std::endl;
    ...
    while (total_steps > 0)
    {
        bool flag = do_opt_gd(uv_new, start_step, start_step + 1000);
        total_steps -= 1000;
        start_step += 1000;
        // bool flag = do_opt_newton(uv_new, 10);
        // total_steps -= 10;
