    ...
    int center)
{
  ...
  if (splits.find(b) != splits.end())
  { // this sector bar is a trace
    if (b.first == center)
      c = std::make_pair(splits[b][0], splits[b][1]);
  ...
  {
    if (b_r.first == center)
      c = std::make_pair(splits[b_r][0], splits[b_r][1]);
    else if (b_r.second == center)
    {
      int n = splits[b_r].size();
      c = std::make_pair(splits[b_r][n - 2], splits[b_r][n - 1]);
    }
  }
  else
  { // this sector bar is a boundary edge
  ...
  for (int k = 0; k < patch_num; k++)
  {
    ...
    { // Tutte generates flips
      ...
      do
      {
        ...
        if (!succ)
        { // greedily push harder in terms of collapsed region to speed up processing
          PV[k] = PV_s;
          kF = kF_s;
          Puv[k] = Puv_s;
          col_eps /= 10;
    ...
    const Eigen::VectorXi &mark)
{
  ...
  decompose_polygon(P, R, c, mark, V2, F2, L);
  ...
  Eigen::MatrixXd H1;
  igl::harmonic(F, T, V2, 1, H1);
  H1.conservativeResize(H1.rows(), 2);
  uv = H1;
  post_processing(V, F, TT, uv);
